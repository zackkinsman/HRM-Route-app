...
def delete_bin(bin_id):
    ...
    try:
        ...
        # Delete only the selected bin
        db.session.delete(bin_entry)
        db.session.commit()

        return jsonify({"message": f"Bin {bin_id} deleted successfully"})
    
    except Exception as e:
        print(f"Error deleting bin: {e}")
        return jsonify({"message": "Error deleting bin"}), 500
